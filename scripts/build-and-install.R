...
version <- unlist(
    strsplit(unlist(strsplit(basename(package_tarball), "SEW_")), ".tar.gz")
)

## move tarball to releases
release_package_tarball <- file.path("releases", paste0("SEW_", version, ".tar.gz"))
...
## build PDF
pdf_name <- file.path("releases", paste0("SEW_", version, ".pdf"))
args = c(
