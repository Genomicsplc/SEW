...
then
    if ! [ -e "SEW/tests/testthat/test-acceptance-${what_to_test}.R" ] && ! [ -e "SEW/tests/testthat/test-unit-${what_to_test}.R" ]
    then
...
# --slave
R -e 'devtools::document("SEW"); devtools::test("SEW", filter = "'${what_to_test}'", reporter = "summary")' 2>&1 | tee ${logfile}

# somehow this gives 0 exit code on parse failure
started_if_1=`cat ${logfile} | grep 'Testing SEW' | wc -l`
